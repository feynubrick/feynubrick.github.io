...
본래 스타일이라는게 주관적이라 옳고 그른 것은 없습니다.
하지만 모든 것을 완전히 다 바꾸기는 어렵기 때문에 여러가지 규칙을 미리 정해둔 모음을 사용할 것입니다.

다시 말하지만, 어떤 규칙을 사용하셔도 상관 없습니다.
각자에게 맞는 규칙을 사용하시면 됩니다.
여기서는 "Prettier"라는 규칙을 적용하는 방법에 대해 알아보겠습니다.
...
```json
{
  ...
  "parserOptions": {
    "ecmaVersion": 6, // ES6 문법을 사용할 수 있습니다
  ...
  "env": {
    "es6": true, // ES6의 전역변수를 사용할 수 있습니다
    "browser": true, // DOM 사용가능
    "node": true
  },
  "rules": {
    "prettier/prettier": [
...
"rules": {
    ...
    "no-console": "off",
    "semi": "always",
    "no-undef": "warn" // 정의되지 않은 것이 나오면 경고(warning)만 띄우게 한다
  },
...
```
...
`lint-staged`를 커밋될 때 사용할 것이니까 Husky 설정도 바꿔줘야겠죠?
...
자 이제 ESLint와 Prettier, Husky와 Lint-staged를 사용해 
