다음 내용은 [이 글](https://weeklywebwisdom.com/2017/09/08/javascript-execution-context-and-scope/)을 한국어로 번역한 것이다.
...
실행 컨텍스트를 생각할 때 발생하는 혼란의 주 근원지 중 하나는 자바스크립트에 블록 수준의 실행 컨텍스트가 없다는 것이다. 예를 들어, C++, Java, C# 등 다른 주류 언어에서, for 문 (또는 while이나 다른 블록) 안에 변수를 선언하면, 그 변수를 블록이 끝나고 나서는 접근할 수 없다(그 블록의 스코프에 속해있기 때문). 이런 일은 자바스크립트에서는 일어나지 않는다. 따라서 위 예제의 (5) 가 헷갈릴 수 있다.
...
이제 첫번째 예를 살펴보자. 이제는 여기나온 것들이 이해될 것이다.
